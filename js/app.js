...
(function () {
  ...
  // screenScale ضریب transform صفحه است؛ برای تبدیل مختصات لمس به میلی‌متر
  // باید اندازه‌ی CSS هر میلی‌متر هم در آن ضرب شود.
  function screenPxPerMM() {
    return screenScale() * CSS_PX_PER_MM;
  }
  ...
  function syncControls() {
    ...
    if ($('allZoomRange')) {
      $('allZoomRange').value = String(allZoomPct);
      $('allZoomVal').textContent = allZoomPct + '٪';
    }
    updateA53Info();
  ...
  function wireItem(el, item) {
    ...
    el.addEventListener('pointermove', function (ev) {
      ...
      var pxPerMM = screenPxPerMM();
      if (st.mode === 'move') {
        item.x = st.x0 + (ev.clientX - st.sx) / pxPerMM;
        item.y = st.y0 + (ev.clientY - st.sy) / pxPerMM;
        clampItemToPage(item);
    ...
    function centerScreen() {
      ...
      var pxPerMM = screenPxPerMM();
      return {
        x: rect.left + (item.x + item.w / 2) * pxPerMM,
        y: rect.top + (item.y + item.h / 2) * pxPerMM
      };
    }
  ...
  // عکس در زمان جابه‌جایی از صفحه بیرون نمی‌زند؛ برای چرخش، باکس محیطی لحاظ می‌شود.
  function clampItemToPage(item) {
    var p = PAPERS[paper];
    var bound = rotatedBound(item);
    var cx = clamp(item.x + item.w / 2, bound.w / 2, p.w - bound.w / 2);
    var cy = clamp(item.y + item.h / 2, bound.h / 2, p.h - bound.h / 2);
    item.x = cx - item.w / 2;
    item.y = cy - item.h / 2;
  }
  ...
  function setAllZoomPreview(value) {
    allZoomPct = clamp(parseInt(value, 10) || 100, 100, 200);
    $('allZoomRange').value = String(allZoomPct);
    $('allZoomVal').textContent = allZoomPct + '٪';
  }

  function applyAllZoom() {
    if (!photos.length) { toast('ابتدا عکس اضافه کنید'); return; }
    var target = allZoomPct / 100;
    var changed = false;
    pages.forEach(function (pg) {
      pg.items.forEach(function (item) {
        if (Math.abs((item.scale || 1) - target) > 0.001) changed = true;
        item.scale = target;
      });
    });
    if (changed) commit();
    ...
    closeToolMenu();
    toast('بزرگ‌نمایی ' + allZoomPct + '٪ روی همه‌ی عکس‌ها اعمال شد');
  }
  ...
  function bind() {
    ...
    $('topMenuBtn').addEventListener('click', function () {
      if ($('toolMenu').hidden) openToolMenu(); else closeToolMenu();
    });
    $('toolClose').addEventListener('click', closeToolMenu);
    $('toolBackdrop').addEventListener('click', closeToolMenu);
    $('allZoomRange').addEventListener('input', function (e) {
      setAllZoomPreview(e.target.value);
    });
    $('applyAllZoom').addEventListener('click', applyAllZoom);
    $('resetAllZoom').addEventListener('click', function () {
      setAllZoomPreview(100);
      applyAllZoom();
    });
