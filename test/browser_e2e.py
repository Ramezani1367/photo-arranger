...
with sync_playwright() as pw:
    ...
    ch_ = page.locator("#page .item .ih.crop").nth(1).bounding_box()
    ccx, ccy = ch_["x"] + ch_["width"] / 2, ch_["y"] + ch_["height"] / 2
    item_center = page.locator("#page .item").nth(1).bounding_box()
    icx = item_center["x"] + item_center["width"] / 2
    icy = item_center["y"] + item_center["height"] / 2
    page.mouse.move(ccx, ccy); page.mouse.down(); page.mouse.move(icx, icy, steps=10); page.mouse.up()
    page.wait_for_timeout(250)
    ...
    dx_mm = abs(after["x"] - before["x"])
    dy_mm = abs(after["y"] - before["y"])
    check(dx_mm > 1 or dy_mm > 1,
          f"جابه‌جایی کار کرد ({before['x']:.2f},{before['y']:.2f} → {after['x']:.2f},{after['y']:.2f})")
    check(dx_mm < 30 and dy_mm < 15,
          f"سرعت درگ با مقیاس صفحه هماهنگ است (جابجایی {dx_mm:.1f}×{dy_mm:.1f}mm)")

    # کشیدن به گوشه‌ی صفحه نباید عکس را از کاغذ بیرون ببرد.
    page_box = page.locator("#page").bounding_box()
    edge_box = page.locator("#page .item").nth(2).bounding_box()
    page.mouse.move(edge_box["x"] + edge_box["width"] / 2, edge_box["y"] + edge_box["height"] / 2)
    page.mouse.down()
    page.mouse.move(page_box["x"] + 2, page_box["y"] + 2, steps=8)
    page.mouse.up()
    page.wait_for_timeout(200)
    edge_after = box_mm(2)
    check(edge_after["x"] >= -0.1 and edge_after["y"] >= -0.1,
          f"عکس داخل مرز کاغذ نگه داشته شد ({edge_after['x']:.2f},{edge_after['y']:.2f}mm)")

    # منوی ابزارهای عمومی و بزرگ‌نمایی درصدی همه‌ی عکس‌ها
    page.locator("#topMenuBtn").click()
    page.locator("#allZoomRange").fill("135")
    check(page.locator("#allZoomVal").inner_text() == "۱۳۵٪" or
          page.locator("#allZoomVal").inner_text() == "135٪", "درصد بزرگ‌نمایی در منوی بالا تغییر کرد")
    page.locator("#applyAllZoom").click()
    page.wait_for_timeout(250)
    zoom_matrix = page.evaluate("() => getComputedStyle(document.querySelector('#page .item img')).transform")
    check(zoom_matrix.startswith("matrix(1.35") or zoom_matrix.startswith("matrix3d(1.35"),
          f"بزرگ‌نمایی درصدی روی همه‌ی عکس‌ها اعمال شد ({zoom_matrix})")
